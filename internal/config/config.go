...
const (
	...
	defaultDBPath                = "./data/music-utils.db"
...
func Load() Config {
	return Config{
		...
		DBPath:                valueOrDefault("DB_PATH", defaultDBPath),
...
func (c Config) Validate() error {
	...
	if c.LRCLIBTimeoutMS < 1 {
		return fmt.Errorf("LRCLIB_TIMEOUT_MS must be positive")
	}
	if strings.TrimSpace(c.LRCLIBUserAgent) == "" {
		return fmt.Errorf("LRCLIB_USER_AGENT must not be empty")
	}
	baseURL, err := url.Parse(strings.TrimSpace(c.LRCLIBBaseURL))
...
func validateEnvironment() error {
	...
	for _, name := range []string{"DB_MAX_OPEN_CONNS", "RATE_LIMIT_PER_SEC", "RATE_LIMIT_PER_MIN", "LRCLIB_TIMEOUT_MS"} {
	...
	for _, name := range []string{"TRUST_PROXY", "LRCLIB_FALLBACK_ENABLED"} {
